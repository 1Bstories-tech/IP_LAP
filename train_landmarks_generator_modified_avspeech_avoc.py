...
parser.add_argument(
    "--pre_audio_root", default="...../Dataset/lrs2_preprocessed_audio", help="root path for preprocessed  audio"
)
parser.add_argument(
    "--landmarks_root", default="...../Dataset/lrs2_landmarks", help="root path for preprocessed  landmarks"
)
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            for idx in range(T):
                ...
                T_content_landmarks[idx] = sorted(
                    T_content_landmarks[idx], key=lambda land_tuple: ori_sequence_idx.index(land_tuple[0])
                )
                ...
                T_content[idx, 0, :] = torch.FloatTensor(
                    [T_content_landmarks[idx][i][1] for i in range(len(T_content_landmarks[idx]))]
                )  # x
                T_content[idx, 1, :] = torch.FloatTensor(
                    [T_content_landmarks[idx][i][2] for i in range(len(T_content_landmarks[idx]))]
                )  # y
            # 03. get T audio
            try:
                audio_mel = np.load(join(args.pre_audio_root, '.'.join(vid_name.split('.')[:-2]), "audio.npy"))
...
def save_checkpoint(model, optimizer, step, checkpoint_dir, epoch, prefix=""):
    checkpoint_path = join(checkpoint_dir, "{}_epoch_{}_checkpoint_step{:09d}.pth".format(prefix, epoch, global_step))
...
def evaluate(model, val_data_loader):
    ...
    for epoch in tqdm(range(eval_epochs), total=eval_epochs):
        ...
        for step, (T_mels, T_pose, T_content, Nl_pose, Nl_content) in prog_bar:
            model.eval()
            T_mels, T_pose, T_content, Nl_pose, Nl_content = (
                T_mels.cuda(non_blocking=True),
                T_pose.cuda(non_blocking=True),
                T_content.cuda(non_blocking=True),
                Nl_pose.cuda(non_blocking=True),
                Nl_content.cuda(non_blocking=True),
            )
...
if __name__ == "__main__":
    ...
    val_data_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=batch_size_val, shuffle=True, drop_last=True, num_workers=num_workers, pin_memory=True
    )
    while global_epoch < 9999999999:
        ...
        for step, (T_mels, T_pose, T_content, Nl_pose, Nl_content) in prog_bar:
            ...
            predict_content = model(T_mels, T_pose, Nl_pose, Nl_content)  # (B*T,2,57)
            T_content = torch.cat(
                [T_content[i] for i in range(T_content.size(0))], dim=0
            )  # (B*T,2,57):ground truth lip and jaw landmarks
