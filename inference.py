...
from models import Renderer, audio


def summarize_landmark(edge_set):
    """Summarize all ficial landmarks used to construct edge

    Args:
        edge_set (set): set of edges

    Returns:
        set: set of landmarks
    """
    landmarks = set()
    for a, b in edge_set:
        landmarks.add(a)
...
def read_video_frames(input_video_path):
    """Read video frames from input video path

    Args:
        input_video_path (str): path of input video

    Returns:
        ori_background_frames (list): list of frames
...
def extract_audio(input_audio_path, fps, mel_step_size):
    """Extract audio from input audio path

    Args:
        input_audio_path (str): path of input audio
        fps (int): fps of video
        mel_step_size (int): mel step size

    Returns:
        mel_chunks (list): list of mel chunks
    """
...
def get_bounding_boxes_and_lip_dist(ori_background_frames, face_mesh, lip_index):
    """Get bounding boxes and lip dist

    Args:
        ori_background_frames (list): list of frames
        face_mesh (mediapipe): mediapipe face mesh
        lip_index (list): the index of the midpoints of the upper lip and lower lip

    Returns:
        boxes (list): list of bounding boxes
        lip_dists (list): list of lip dist
    """
    boxes = []  # bounding boxes of human face
    lip_dists = []  # we define the lip dist(openness): distance between the  midpoints of the upper lip and lower lip
    for frame_idx, full_frame in tqdm(
        enumerate(ori_background_frames),
        total=len(ori_background_frames),
        desc="Getting bounding boxes and lip distances",
    ):
    ...
    return boxes, lip_dists


def crop_face(ori_background_frames, boxes):
    ...
    Returns:
        face_crop_results (list): list of cropped faces
    """
    face_crop_results = [
        [image[y1:y2, x1:x2], (y1, y2, x1, x2)] for image, (y1, y2, x1, x2) in zip(ori_background_frames, boxes)
    ]
    return face_crop_results


def detect_facial_landmarks(ori_background_frames, face_mesh, face_crop_results):
    all_pose_landmarks, all_content_landmarks = [], []  # content landmarks include lip and jaw landmarks
    for frame_idx, full_frame in tqdm(
        enumerate(ori_background_frames), total=len(ori_background_frames), desc="Detecting facial landmarks"
    ):
        h, w = full_frame.shape[0], full_frame.shape[1]
        results = face_mesh.process(cv2.cvtColor(full_frame, cv2.COLOR_BGR2RGB))
        if not results.multi_face_landmarks:
    ...
    return all_pose_landmarks, all_content_landmarks
...
def get_landmarks(ori_background_frames, lip_index):
    """Get landmarks
    ...
    Returns:
        all_pose_landmarks (list): list of pose landmarks
        all_content_landmarks (list): list of content landmarks
        lip_dists (list): list of lip distances
        face_crop_results (list): list of cropped faces
    """
    with mp_face_mesh.FaceMesh(
        ...
        boxes, lip_dists = get_bounding_boxes_and_lip_dist(ori_background_frames, face_mesh, lip_index)

        # (2) croppd face
        face_crop_results = crop_face(ori_background_frames, boxes)

        # (3) detect facial landmarks
        all_pose_landmarks, all_content_landmarks = detect_facial_landmarks(
            ori_background_frames, face_mesh, face_crop_results
        )
    ...
    return all_pose_landmarks, all_content_landmarks, lip_dists, face_crop_results


def get_reference_landmarks(all_pose_landmarks, all_content_landmarks, lip_dists):
    ...
    Returns:
        Nl_pose (tensor): tensor of pose landmarks
        Nl_content (tensor): tensor of content landmarks
        lip_dist_idx (list): list of lip distances' indeces sorted by lip openness
    """
    dists_sorted = sorted(lip_dists, key=lambda x: x[1])
    lip_dist_idx = np.asarray([idx for idx, _ in dists_sorted])  # the frame idxs sorted by lip openness
    ...
    for idx in range(Nl):
        ...
        Nl_pose_landmarks[idx] = sorted(
            Nl_pose_landmarks[idx], key=lambda land_tuple: ori_sequence_idx.index(land_tuple[0])
        )
        Nl_content_landmarks[idx] = sorted(
            Nl_content_landmarks[idx], key=lambda land_tuple: ori_sequence_idx.index(land_tuple[0])
        )

        Nl_pose[idx, 0, :] = torch.FloatTensor(
            [Nl_pose_landmarks[idx][i][1] for i in range(len(Nl_pose_landmarks[idx]))]
        )  # x
        Nl_pose[idx, 1, :] = torch.FloatTensor(
            [Nl_pose_landmarks[idx][i][2] for i in range(len(Nl_pose_landmarks[idx]))]
        )  # y
        Nl_content[idx, 0, :] = torch.FloatTensor(
            [Nl_content_landmarks[idx][i][1] for i in range(len(Nl_content_landmarks[idx]))]
        )  # x
        Nl_content[idx, 1, :] = torch.FloatTensor(
            [Nl_content_landmarks[idx][i][2] for i in range(len(Nl_content_landmarks[idx]))]
        )  # y
    Nl_content = Nl_content.unsqueeze(0)  # (1,Nl, 2, 57)
    Nl_pose = Nl_pose.unsqueeze(0)  # (1,Nl,2,74)
    return Nl_pose, Nl_content, lip_dist_idx


def get_reference_images(face_crop_results, all_pose_landmarks, all_content_landmarks, lip_dist_idx, ref_img_N):
    ...
    Args:
        ...
        all_content_landmarks (list): list of content landmarks
        lip_dist_idx (list): list of lip distances' indeces sorted by lip openness
        ref_img_N (int): number of reference images

    Returns:
        ref_img_full_face_landmarks (tensor): tensor of full face landmarks
        ref_imgs (tensor): tensor of reference images
        ref_imgs_raw (list): list of reference images
    """
    ref_img_idx = [int(lip_dist_idx[int(i)]) for i in torch.linspace(0, input_vid_len - 1, steps=ref_img_N)]
    ref_imgs_raw = [face_crop_results[idx][0] for idx in ref_img_idx]
    ...
    for idx in range(ref_img_N):
        ref_img_pose_landmarks[idx] = sorted(
            ref_img_pose_landmarks[idx], key=lambda land_tuple: ori_sequence_idx.index(land_tuple[0])
        )
        ref_img_content_landmarks[idx] = sorted(
            ref_img_content_landmarks[idx], key=lambda land_tuple: ori_sequence_idx.index(land_tuple[0])
        )
        ref_img_pose[idx, 0, :] = torch.FloatTensor(
            [ref_img_pose_landmarks[idx][i][1] for i in range(len(ref_img_pose_landmarks[idx]))]
        )  # x
        ref_img_pose[idx, 1, :] = torch.FloatTensor(
            [ref_img_pose_landmarks[idx][i][2] for i in range(len(ref_img_pose_landmarks[idx]))]
        )  # y

        ref_img_content[idx, 0, :] = torch.FloatTensor(
            [ref_img_content_landmarks[idx][i][1] for i in range(len(ref_img_content_landmarks[idx]))]
        )  # x
        ref_img_content[idx, 1, :] = torch.FloatTensor(
            [ref_img_content_landmarks[idx][i][2] for i in range(len(ref_img_content_landmarks[idx]))]
        )  # y

    ref_img_full_face_landmarks = torch.cat([ref_img_pose, ref_img_content], dim=2).cpu().numpy()  # (N,2,131)

    ref_imgs = [cv2.resize(face.copy(), (img_size, img_size)) for face in ref_imgs_raw]
    ref_imgs = torch.FloatTensor(np.asarray(ref_imgs) / 255.0).unsqueeze(0).permute(0, 1, 4, 2, 3).cuda()
    # (1,N,3,H,W)

    return ref_img_full_face_landmarks, ref_imgs, ref_imgs_raw
...
def get_reference_sketches(ref_img_full_face_landmarks, ref_imgs_raw, facemesh_connection):
    """Get reference sketches

    Args:
        ref_img_full_face_landmarks (tensor): tensor of full face landmarks
        ref_imgs_raw (list): list of reference images
        facemesh_connection (set): set of facemesh connections

    Returns:
        ref_img_sketches (tensor): tensor of reference sketches
    """
    ref_img_sketches = []
    for frame_idx in range(ref_img_full_face_landmarks.shape[0]):  # N
        full_landmarks = ref_img_full_face_landmarks[frame_idx]  # (2,131)
        h, w = ref_imgs_raw[frame_idx].shape[0], ref_imgs_raw[frame_idx].shape[1]
    ...
    ref_img_sketches = (
        torch.FloatTensor(np.asarray(ref_img_sketches) / 255.0).cuda().unsqueeze(0).permute(0, 1, 4, 2, 3)
    )
    # (1,N, 3, 128, 128)
    return ref_img_sketches


def prepare_audio_to_landmark_input(
    batch_start_idx, T, mel_chunks, all_pose_landmarks, input_frame_sequence, ori_sequence_idx
):
    T_mel_batch, T_pose_landmarks = [], []
    ...
    # prepare pose landmarks tensor
    T_pose = torch.zeros((T, 2, 74))  # 74 landmark
    for idx in range(T):
        T_pose_landmarks[idx] = sorted(
            T_pose_landmarks[idx], key=lambda land_tuple: ori_sequence_idx.index(land_tuple[0])
        )
        T_pose[idx, 0, :] = torch.FloatTensor(
            [T_pose_landmarks[idx][i][1] for i in range(len(T_pose_landmarks[idx]))]
        )  # x
        T_pose[idx, 1, :] = torch.FloatTensor(
            [T_pose_landmarks[idx][i][2] for i in range(len(T_pose_landmarks[idx]))]
        )  # y
    T_pose = T_pose.unsqueeze(0)  # (1,T, 2,74)

    return T_mels, T_pose


def prepare_alignment_and_translation_input(
    T_pose, predict_content, facemesh_connection, batch_start_idx, input_frame_sequence, face_crop_results
):
    # 1. prepare full face landmarks
    ...
    # 2. draw target sketch
    T_crop_face = []
    for mel_chunk_idx in range(batch_start_idx, batch_start_idx + T):  # for each T frame
        input_frame_idx = int(input_frame_sequence[mel_chunk_idx])
        face, _ = face_crop_results[input_frame_idx]
    ...
    target_sketches = T_target_sketches.unsqueeze(0)  # (1,T,3,128, 128)

    # 3.lower-half masked face
    ori_face_img = (
        torch.FloatTensor(cv2.resize(T_crop_face[2], (img_size, img_size)) / 255)
        .permute(2, 0, 1)
        .unsqueeze(0)
        .unsqueeze(0)
    )  # (1,1,3,H, W)

    return target_sketches, ori_face_img
...
def paste_generated_face(
    gen_face, batch_start_idx, T, input_frame_sequence, ori_background_frames, face_crop_results, fa
):
...
def generate_final_face_image_and_output_video(
    mel_chunks,
    input_vid_len,
    T,
    face_crop_results,
    ...
    # the input template video may be shorter than audio in this case we repeat the input template video as following
    ...
    for batch_start_idx in tqdm(
        range(0, input_mel_chunks_len - 2, 1),
        ...
        T_mels, T_pose = prepare_audio_to_landmark_input(
            batch_start_idx, T, mel_chunks, all_pose_landmarks, input_frame_sequence, ori_sequence_idx
        )
        ...
        # (2) for each batch of T frame, generate corresponding face using renderer
        target_sketches, ori_face_img = prepare_alignment_and_translation_input(
            T_pose, predict_content, facemesh_connection, batch_start_idx, input_frame_sequence, face_crop_results
        )
        ...
        # return  (1,3,H,W)
        target_sketches, ori_face_img = target_sketches.cuda(), ori_face_img.cuda()
        with torch.no_grad():
            generated_face, _, _, _ = renderer(
        ...
        # (3) paste generated face into background
        full = paste_generated_face(
            gen_face, batch_start_idx, T, input_frame_sequence, ori_background_frames, face_crop_results, fa
        )
...
def ip_lap_model(
    ...
    # Get landmarks
    all_pose_landmarks, all_content_landmarks, lip_dists, face_crop_results = get_landmarks(
        ori_background_frames, lip_index
    )

    # Get reference landmarks: select N_l reference landmarks for landmark transformer according to lip openness
    Nl_pose, Nl_content, lip_dist_idx = get_reference_landmarks(all_pose_landmarks, all_content_landmarks, lip_dists)

    # Get reference images: select reference images for rendering according to lip openness
    ref_img_full_face_landmarks, ref_imgs, ref_imgs_raw = get_reference_images(
        face_crop_results, all_pose_landmarks, all_content_landmarks, lip_dist_idx, ref_img_N
    )

    # Get reference sketches: select reference sketches for rendering according to lip openness
    ref_img_sketches = get_reference_sketches(ref_img_full_face_landmarks, ref_imgs_raw, facemesh_connection)

    # Generate final face image and output video
    return generate_final_face_image_and_output_video(
        mel_chunks,
        input_vid_len,
        T,
        face_crop_results,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--landmark_gen_checkpoint_path", type=str, default="./test/checkpoints/landmarkgenerator_checkpoint.pth"
    )
    ...
    # Set constants
    ref_img_N = 25  # number of reference images
    Nl = 15  # number of reference landmarks
    T = 5  # number of landmarks to generate
    mel_step_size = 16  # number of mel spectrogram timesteps
    img_size = 128  # face image size
    ...
    outfile_path = os.path.join(
        output_dir, "{}_N_{}_Nl_{}.mp4".format(input_video_path.split("/")[-1][:-4] + "result", ref_img_N, Nl)
    )

    # Set mediapipe and face_alignment
    ...
    full_face_landmark_sequence = [
        *list(range(0, 4)),
        *list(range(21, 25)),
        *list(range(25, 91)),  # upper-half face
        *list(range(4, 21)),  # jaw
        *list(range(91, 131)),
    ]  # mouth
    ...
    # Read video frames
    ori_background_frames, fps = read_video_frames(input_video_path)
    input_vid_len = len(ori_background_frames)

    # Extract audio
    mel_chunks = extract_audio(input_audio_path, fps, mel_step_size)

    # Prepare output video stream
    frame_h, frame_w = ori_background_frames[0].shape[:-1]
    out_stream = cv2.VideoWriter(
        "{}/result.avi".format(temp_dir), cv2.VideoWriter_fourcc(*"DIVX"), fps, (frame_w, frame_h)
    )

    for frame in ip_lap_model(
        ori_background_frames,
        mel_chunks,
        input_vid_len,
        T,
    ...
    # Post-process output video
    command = "ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}".format(
        input_audio_path, "{}/result.avi".format(temp_dir), outfile_path
    )
    subprocess.call(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
