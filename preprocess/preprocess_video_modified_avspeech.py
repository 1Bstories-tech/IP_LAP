...
pose_landmark_idx = summarize_landmarks(
    FACEMESH_NOSE.union(
        *[
            FACEMESH_RIGHT_EYEBROW,
            FACEMESH_RIGHT_EYE,
...
def process_video_file(mp4_path):
    vidname = '.'.join(os.path.basename(mp4_path).split(".")[:-1])
    dirname = '_'.join(vidname.split("_")[:-2])
    out_path = f"{dirname}/{vidname}"
    with mp_face_mesh.FaceMesh(
        static_image_mode=True, max_num_faces=1, refine_landmarks=True, min_detection_confidence=0.5
    ) as face_mesh:
        video_stream = cv2.VideoCapture(mp4_path)
        fps = round(video_stream.get(cv2.CAP_PROP_FPS))
        if fps != 25 and fps != 24:
        ...
        for frame_idx, full_frame in enumerate(frames):
            ...
            face_frame = cv2.resize(
                full_frame[int(y_min * h) : int(y_max * h), int(x_min * w) : int(x_max * w)], (128, 128)
            )
            ...
            out_dir = os.path.join(output_landmark_root, out_path)
            ...
            out_dir = os.path.join(output_sketch_root, out_path)
            os.makedirs(out_dir, exist_ok=True)
            cv2.imwrite(os.path.join(out_dir, str(frame_idx) + ".png"), annotated_image)

            # save face frame
            out_dir = os.path.join(output_face_root, out_path)
            os.makedirs(out_dir, exist_ok=True)
...
def main():
    ...
    mp4_list = glob.glob(path.join(input_mp4_root, "*.mp4"))  # example: .../lrs2_video/5536038039829982468/00001.mp4
